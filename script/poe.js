...
async function main() {
  ...
  if (error) { $done({ content: "Network Error", backgroundColor: "#6C5CE7" }); return; }
  const s = response ? (response.status || response.statusCode || 0) : 0;
  if (!((s >= 200 && s < 500) || s === 401 || s === 403)) {
    $done({ content: "Not Available", backgroundColor: "#6C5CE7" }); return;
  }
  ...
  $done({ content: cc ? "Available \u00b7 " + cc : "Available", backgroundColor: "#6C5CE7" });
}
(async () => { main().catch(() => $done({ content: "Error", backgroundColor: "#6C5CE7" })); })();