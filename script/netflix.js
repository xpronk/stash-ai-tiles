// Netflix availability tile for Stash
var TITLE = "Netflix";
var CHECK_URL = "https://www.netflix.com/title/81215567";
...
function checkService(country) {
  ...
  var text = data || '';
  var ok = !error && (code === 200 || code === 301 || code === 302);
  var only = ok && (text.indexOf('Not Available') >= 0 || text.indexOf('unavailable') >= 0);
  var blocked = error || code === 403 || code === 451;
  var label = blocked ? '❌ 不可用' : (only ? '⚠️ 仅自制' : (ok ? '✅ 解锁' : '⚠️ 待确认'));
