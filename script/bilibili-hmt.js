...
// https://api.bilibili.com/pgc/season/index/result?season_type=1&pagesize=1&type=1 and & replaced by build script
async function main() {
  const tik = Date.now();
  const [cc, sr] = await Promise.all([
    getCountry(),
    request("GET", "https://api.bilibili.com/pgc/season/index/result?season_type=1&pagesize=1&type=1" + "&" + "_=" + tik)
  ]);
